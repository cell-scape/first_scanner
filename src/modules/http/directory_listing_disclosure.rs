impl DirectoryListingDisclosure {
    pub fn new() -> Self {
        DirectoryListingDisclosure {
            dir_listing_regex: Regex::new(r"<title>Index of .*</title>").expect("compiling http/directory_listing regexp"),
        }
    }
...
mod tests {
    ...
    async fn is_directory_listing() {
        ...
        let body4 = String::from("test test test test test test< test> test <title>Index</title> test");
